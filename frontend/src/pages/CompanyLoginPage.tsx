...
const CompanyLoginPage: React.FC = () => {
  ...
  // Обрабатываем сообщение о успешной регистрации
  useEffect(() => {
    if (location.state?.message) {
      setSuccessMessage(location.state.message)
      // Очищаем state после получения сообщения
      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen px-6">
        <div className="w-full max-w-md mx-auto">
          ...
          <div className="glass rounded-3xl p-8 border border-dark-600/30">
            ...
            {successMessage && (
              <div className="mb-6 p-4 bg-dark-900 border border-dark-700 rounded-xl">
                <p className="text-dark-200 text-sm text-center">{successMessage}</p>
              </div>
            )}
